function results = perceptRunBlock(p, feedback, conf, ntrials, staircase_reversal, stepsize, adapt, start_x, phase,indx)
...
while nreversals < staircase_reversal && i_trial < ntrials
    ...
    if phase == 1 && num == trial
        if p.condition{1} == '1'
            img = append('./OASIS Photos/positive/',int2str(indx(1)),'.jpg');
        elseif p.condition{1} == '2'
            img = append('./OASIS Photos/neutral/',int2str(indx(1)),'.jpg');
        elseif p.condition{1} == '3'
            img = append('./OASIS Photos/negative/',int2str(indx(1)),'.jpg');
        end
        indx(1)=[];
        Screen('Flip',p.frame.ptr);
        ima = imread(img,'jpg');
        Screen('PutImage',p.frame.ptr,ima);
        Screen('Flip',p.frame.ptr);
        WaitSecs(2);
        r(1) = [];
        if isempty(r) == 0
